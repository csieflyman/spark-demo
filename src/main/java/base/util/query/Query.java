...
public class Query {
    ...
    private static final String Q_PAGE_NO = "q_pageNo";
    private static final String Q_PAGE_SIZE = "q_pageSize";
    private static final String Q_ORDER_BY = "q_orderBy";
    private static final String Q_COUNT = "q_count";
    private static final String Q_PREDICATES = "q_predicates";
    private static final String Q_OR = "q_or";
    public static final String Q_FETCH_RELATIONS = "q_relations";
    private static final String Q_FETCH_FIELDS = "q_fields";

    private DateTimeFormatter dateTimeFormatter = DateTimeUtils.UTC_DATE_TIME_FORMATTER;
    private DateTimeFormatter dateFormatter = DateTimeUtils.UTC_DATE_FORMATTER;
    ...
    private void put(String param, String value) {
        ...
        switch (param) {
            ...
            case Q_FETCH_RELATIONS:
                fetchRelations(Sets.newLinkedHashSet(Splitter.on(",").split(value))
                        .stream().map(String::trim).collect(Collectors.toCollection(LinkedHashSet::new)));
                break;
            case Q_FETCH_FIELDS:
                fetchProperties(Sets.newLinkedHashSet(Splitter.on(",").split(value))
                        .stream().map(String::trim).collect(Collectors.toCollection(LinkedHashSet::new)));
                break;
            default:
    ...
    private Object tryToConvertToDate(String s) {
        try {
            return s.length() == DateTimeUtils.DATE_PATTERN.length() ?
                    Date.from(ZonedDateTime.parse(s, dateFormatter).toInstant()) :
                    Date.from(ZonedDateTime.parse(s, dateTimeFormatter).toInstant());
